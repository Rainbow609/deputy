...
- [x] 2.1 删除 `get_node_list.py`
- [x] 2.2 删除 `proxy_providers/` 整个目录 (11 个 yaml)
- [x] 2.3 删除 `rule_providers/` 整个目录 (14 个 yaml)
- [x] 2.4 删除 `templates/` 整个目录 (6 个 yaml: head.yaml / head_mobile.yaml / proxy-providers.yaml / proxy-providers_baipiao.yaml / rules_group.yaml / rules_group_mobile.yaml)

## 3. 老管线输出配置删除

- [x] 3.1 删除 `clash_config_v3.yaml`
- [x] 3.2 删除 `config_mobile.yaml`
- [x] 3.3 删除 `config_mobile_baipiao.yaml`
- [x] 3.4 删除 `config_magisk.yaml`

## 4. 老管线 workflow 删除

- [x] 4.1 删除 `.github/workflows/schedule-get-node-list.yml`

## 5. config.template.yaml typo 修复

- [x] 5.1 第 15 行 `path: ./rule_provider/AWAvenue-Ads.yaml` → `path: ./rule_providers/AWAvenue-Ads.yaml`
...
- [x] 8.1 `git add -A` + `git commit -m "chore(deputy): retire legacy get_node_list.py pipeline, keep only nodes.toml → sync_nodes.py"`
