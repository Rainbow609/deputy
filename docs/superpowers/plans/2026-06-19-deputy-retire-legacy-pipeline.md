...
- [ ] **Step 3: 验证 .github/workflows/ 仍含 sync-and-release.yml**
...
- [ ] **Step 2: smoke render — `scripts/sync_nodes.py` 跑通**
...
- [ ] Task 1 grep audit 通过（仅命中待删除目标）
- [ ] Task 2 `.github/workflows/schedule-get-node-list.yml` 已物理删除
- [ ] Task 3 `get_node_list.py` 已删除
- [ ] Task 4 `proxy_providers/`、`rule_providers/`、`templates/` 三个目录已删除
- [ ] Task 5 4 个老输出配置已删除
- [ ] Task 6 `config.template.yaml` 仅 3 行 path 字段变更
- [ ] Task 7 grep 0 匹配 + smoke render exit=0 + pytest 全绿
- [ ] Task 8 单 commit 已落地，commit message 准确
